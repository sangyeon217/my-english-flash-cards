...
npm run backup   # cards 를 backups/cards-<timestamp>.json 으로 덤프 (.env.local 의 DATABASE_URL 사용)
```
...
테스트 러너는 설정돼 있지 않다. `src/lib/highlight.ts` 는 작고 함수형이라(다만 `compromise`
의존성이 있음), Node 26 의 네이티브 TypeScript 지원으로 직접 실행할 수 있다: `.ts` 파일을
import 하는 `node test.mjs` (node 가 `node_modules` 에서 `compromise` 를 해소하므로 프로젝트
루트에서 실행하면 됨).
...
- **`scripts/backup-cards.mjs` — DB 백업(export).** DB 유실 대비용 standalone ESM
  스크립트. 앱 계층에 의존하지 않고 `@neondatabase/serverless` 로 직접 `cards` 를 조회해
  `Card` 계약(`createdAt` epoch ms 포함)대로 매핑, `backups/cards-<timestamp>.json` 으로
  저장한다(`getAllCards` 와 동일 정렬). `.env.local` 의 `DATABASE_URL` 을 쓴다. 새 파일이
  안전히 저장된 뒤에만 이전 `cards-<timestamp>.json` 들을 삭제해 **최신 1개만 유지**한다
  (쓰기/DB 실패 시 기존 백업은 보존). `npm run backup` 으로 실행하고 주기 실행은
  로컬 cron/launchd 로 사용자가 설정한다. `backups/` 는 단어 데이터가 public 레포에
  노출되지 않도록 `.gitignore` 됨.

- **`src/lib/db/` — 데이터베이스 계층 (`server-only`).** `schema.ts` 가 `cards` 테이블을
  정의 (uuid PK `defaultRandom`, `card_status` pgEnum, `favorite` boolean(기본 false),
  ...
  로 변환해 `Card.createdAt: number` 계약을 보존한다. `getAllCards` 는 즐겨찾기(`favorite`)를
  먼저, 그 안에서는 최신순(`created_at` desc)으로 정렬한다. `drizzle.config.ts`(루트)가
...
- **`src/hooks/useCards.ts` — 유일한 상태 소유자.** `useEffect` 에서 카드를 로드하고,
  ...
  감싼다 (`addCard / editCard / toggleStatus / toggleFavorite / removeCard` — 낙관적 React 상태 업데이트
  *및* repository 호출). 필터링된 목록은 즐겨찾기를 상단에 우선 정렬하는 안정 정렬을 거치므로
  `toggleFavorite` 의 낙관적 갱신 후에도 순서가 즉시 반영된다. 홈 페이지가 유일한 소비자이고,
...
- **`src/lib/highlight.ts` + `HighlightedText.tsx` — 단어 강조.** `splitByWord`(순수,
  ...
  쪼개고, `HighlightedText` 가 매치를 `<mark>` 로 감싼다. 자동 강조는 굴절형까지 인식한다:
  `compromise`(nlp)로 카드 단어의 활용형(규칙 -s/-ed/-ing/-ies + 불규칙형 sweep→swept)을
  생성해 예문에서 **전체 굴절형 토큰**을 강조한다(`embody`→`embodies`). 단어/구를 분기 처리하며,
  구는 정확 일치를 먼저 시도하고 실패 시 자리표시자(`somebody` 등)를 건너뛰고 토큰별 굴절형으로
  매치한다(`weigh somebody down`→`weighed down`). 단, 예문이 다른 어휘를 쓰면(particle 변화 등)
  매치되지 않을 수 있다.

- **`src/lib/speech.ts` — 단어 발음 (client 전용).** 별도 라이브러리 없이 브라우저 내장
  Web Speech API(`window.speechSynthesis`)만 쓴다. `speak(text)` 는 이전 발화를
  `cancel()` 한 뒤 `en-US` `SpeechSynthesisUtterance` 를 재생하고, `pickEnglishVoice()`
  가 `getVoices()` 중 영어 음성(en-US 우선)을 best-effort 로 고른다(없으면 브라우저 기본
  음성 폴백). `isSpeechSupported()` 가드 포함. 앱 상태·DB 와 무관한 stateless 동작이라
  `FlashCard` 가 콜백 없이 직접 호출한다. 음성에 표준 성별 필드가 없어 성별/음성 선택은
  후속 과제로 남김(시그니처는 확장 가능하게 유지).
...
- **`src/components/FlashCard.tsx`** — 클릭 시 앞면(단어 + 강조 예문) ↔ 뒷면(뜻) 뒤집힘.
  ...
  `[backface-visibility:hidden]`)를 쓴다. 앞면 우상단의 액션 버튼(별=즐겨찾기 토글 →
  `onToggleFavorite`, 연필=수정 → `onEdit`, 휴지통=삭제 → `onRemove`)과 단어(`card.word`)
  오른쪽의 스피커 버튼(발음 듣기 → `speak(card.word)` 직접 호출)은 모두
  `e.stopPropagation()` 을 호출해 플립이 트리거되지 않게 한다. 별 아이콘은 `card.favorite` 면
  채워지고(amber), `onToggleFavorite`/`onToggleStatus` 는 `id` 만 넘기지만 `onEdit`/`onRemove`
  는 `Card` 를 통째로 넘긴다 — 홈 페이지는 후자를 받아 즉시 수행하지 않고 수정/삭제 모달의
  대상 state 로 보관한다.
...
- `Card` 스키마(`src/types/card.ts`)는 DB 계층·storage·UI 가 공유하는 계약이다.
  ...
  `status`(기본 `learning`), `favorite`(기본 `false`), `createdAt` 은 insert 시 DB 가 채우고
  `src/lib/db/cards.ts` 에서 `Card` 로 매핑된다.
